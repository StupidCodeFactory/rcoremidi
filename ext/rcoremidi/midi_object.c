...
static VALUE add_entites_to_device(VALUE device, VALUE entities, ItemCount count)
{
        ...
        *midi_entity = MIDIDeviceGetEntity(*get_midi_data(device), count);
        ...
        add_source_to_entity(entity, rb_iv_get(entity, "@endpoints"), MIDIEntityGetNumberOfSources(*midi_entity));
        add_destination_to_entity(entity, rb_iv_get(entity, "@endpoints"), MIDIEntityGetNumberOfDestinations(*midi_entity));

        rb_ary_push(rb_iv_get(device, "@entites"), entity);
...
get_devices(VALUE devices, ItemCount count)
{
        ...
        *midi_device = MIDIGetDevice(count);
        ...
        add_entites_to_device(device, rb_iv_get(device, "@entites"), MIDIDeviceGetNumberOfEntities(*midi_device));
        if (count == 0) {
                return devices;
        } else {
...
static void populate_devices(VALUE klass)
{
        ...
        rb_cvar_set(devices, devices_intern, get_devices(devices, MIDIGetNumberOfDevices()));
}
